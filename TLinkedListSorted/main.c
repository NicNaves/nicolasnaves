...
int main(){
  ...
  printf("\nIniciando com 6 alunos em ordem \n\n");
  while (i<6){
  ...
  printf("\n\n Consulta quem está na posição 5 \n\n");
  if((ret = list_find_pos(list, 5, &aux)) == SUCCESS)
  ...
  printf("\n\n Consulta quem possui matrícula 4  \n\n");
  if((ret = list_find_mat(list,  4, &aux)) == SUCCESS)
    printf("Id: %d \nNome: %s\nNotas: ( %.2f, %.2f, %.2f )\n",aux.id, aux.name, aux.g1, aux.g2,aux.g3);
  else if(ret == INVALID_NULL_POINTER)
    printf("\nPonteiro inválido.");
  else if (ret == ELEM_NOT_FOUND)
  ...
  printf("\n\n Consulta qual posição a matrícula 4 está \n\n");
  if((ret = list_get_pos(list, 4,  &pos)) == SUCCESS)
  ...
  printf("\n\n Remove a posição 2\n\n");
  if((ret = list_erase(list, 1)) == SUCCESS)
    list_print(list);
  else if(ret == INVALID_NULL_POINTER)
    printf("\nPonteiro inválido.");
  else if (ret == ELEM_NOT_FOUND)
